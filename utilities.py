...
class Utilities:
    ...
    def sanity_check(self, sentence, block_size, name: str):
        ...
        _,  attn_maps = self.model(input_tensor,return_map=True) # Ignore the output of the model, and only get the attention maps; make sure your encoder returns the attention maps
        ...
        for j, attn_map in enumerate(attn_maps):
            attn_map = attn_map.squeeze(0).detach().cpu()  # Remove batch dimension and convert to NumPy array

            # Check if the attention probabilities sum to 1 over rows
            total_prob_over_rows = torch.sum(attn_map, dim=1).cpu()
            ...
            # Save the plot
            plt.savefig(f"{name}_attention_map_{j + 1}.png")
            plt.close()
